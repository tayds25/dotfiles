...
# Create an archive (auto compression based on extension)
...
# find packages with apt cache and fzf
...
# Last-2-levels title, with ~ for home
set_bash_title() {
  local dir
  ...
  if [[ $PWD == "$HOME" || $PWD == "$HOME/" ]]; then
    ...
    # Inside $HOME -> "~/<parent>/<base>" (2 levels max)
    local rel=${PWD#$HOME/}
    local base=${rel##*/}                 # last
    if [[ $rel == */* ]]; then
      local parent=${rel%/*}; parent=${parent##*/}   # parent segment
      dir="~/$parent/$base"
    else
      dir="~/$base"
    fi
  else
    # Outside $HOME -> "<parent>/<base>" (2 levels max)
    local base=${PWD##*/}
    if [[ $PWD == */* && ${PWD%/*} != "/" ]]; then
  ...
  # Set both window (OSC 2) and icon/title (OSC 0)
  printf '\033]0;%s\007\033]2;%s\007' "$dir" "$dir"
}

# Prepend safely so it coexists with other PROMPT_COMMAND hooks
PROMPT_COMMAND="set_bash_title${PROMPT_COMMAND:+;$PROMPT_COMMAND}"
# Last-2-levels title, with ~ for home
set_bash_title() {
  local dir
